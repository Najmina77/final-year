...
def get_embu_data_from_2021():
    day_data_url =  'https://power.larc.nasa.gov/api/temporal/daily/point?parameters=T2M,PRECTOTCORR,WS2M&community=AG&longitude=37.4596&latitude=-0.5388&start=20200101&end=20210725&format=JSON'
...
def get_nyeri_data_from_2021():
    day_data_url =  'https://power.larc.nasa.gov/api/temporal/daily/point?parameters=T2M,PRECTOTCORR,WS2M&community=AG&longitude=36.9580&latitude=-0.4371&start=20200101&end=20210725&format=JSON'
    response = requests.get(day_data_url)
    day_data = response.content.decode('utf-8') 
    day_data = json.loads(day_data)
    properties = day_data['properties']
...
def get_nanyuki_data_from_2021():
    day_data_url =  'https://power.larc.nasa.gov/api/temporal/daily/point?parameters=T2M,PRECTOTCORR,WS2M&community=AG&longitude=37.0722&latitude=0.0074&start=20200101&end=20210725&format=JSON'
    response = requests.get(day_data_url)
    day_data = response.content.decode('utf-8') 
    day_data = json.loads(day_data)
    properties = day_data['properties']
...
# shift the current precp to the next day
embu_pred_precp = embu_df.precipitation.to_frame().shift(1).rename(columns = {"precipitation": "precipitation_pred"})
embu_actual_precp = embu_df.precipitation.to_frame().rename(columns = {"precipitation": "precipitation_actual" })
 
# Concatenate the actual and predicted temperature
#embu_one_step_df = pd.concat([embu_actual_df, embu_predicted_df],axis=1)

# Concatenate the actual and predicted precipitation
embu_joined_precipitation_df = pd.concat([embu_actual_precp, embu_pred_precp],axis=1)

# Select from the second row, because there is no prediction for today due to shifting.
#embu_one_step_df = embu_one_step_df[1:]
embu_joined_precipitation_df = embu_joined_precipitation_df[1:]
...
#shift the current precp to the next 
nyeri_pred_precp = nyeri_df.precipitation.to_frame().shift(1).rename(columns = {"precipitation": "precipitation_pred" })
nyeri_actual_precp = nyeri_df.precipitation.to_frame().rename(columns = {"precipitation": "precipitation_actual" })
# Concatenate the actual and predicted temperature

#nyeri_one_step_df = pd.concat([nyeri_actual_df, nyeri_predicted_df],axis=1)

# Concatenate the actual and predicted precipitation
nyeri_jioned_precipitation_df = pd.concat([nyeri_actual_precp, nyeri_pred_precp],axis=1)


# Select from the second row, because there is no prediction for today due to shifting.
#nyeri_one_step_df = nyeri_one_step_df[1:]
nyeri_jioned_precipitation_df =nyeri_jioned_precipitation_df[1:]
...
#shift the current  precpto the next day. 
nanyuki_pred_precp = nanyuki_df.precipitation.to_frame().shift(1).rename(columns = {"precipitation": "precipitation_pred" })
nanyuki_actual_precp = nanyuki_df.precipitation.to_frame().rename(columns = {"precipitation": "precipitation_actual" })

# Concatenate the actual and predicted temperature
#nanyuki_one_step_df = pd.concat([nanyuki_actual_df, nanyuki_predicted_df],axis=1)

# Concatenate the actual and predicted precipitation
nanyuki_jioned_precipitation_df = pd.concat([nanyuki_actual_precp, nanyuki_pred_precp],axis=1)

# Select from the second row, because there is no prediction for today due to shifting.
#nanyuki_one_step_df = nanyuki_jioned_precipitation_df[1:]
nanyuki_jioned_precipitation_df = nanyuki_jioned_precipitation_df[1:]
...
# Fit the SARIMAX model using optimal parameters
print('Trainign the Embu prediction Model')
embu_pred_mod = sm.tsa.statespace.SARIMAX(embu_joined_precipitation_df.precipitation_actual,
                                order=(1, 1, 1),
                                seasonal_order=(1, 1, 1, 12),
                                enforce_stationarity=False,
                                enforce_invertibility=False)

embu_precipitation_results = embu_pred_mod.fit()
...
print('The columns for dfs')
print(nyeri_jioned_precipitation_df.columns)
print(embu_joined_precipitation_df.columns)
print(nanyuki_jioned_precipitation_df.columns)

# Fit the SARIMAX model using optimal parameters
print('Training the Nyeri waether Model')
nyeri_mod = sm.tsa.statespace.SARIMAX(nyeri_jioned_precipitation_df.precipitation_actual,
                                order=(1, 1, 1),
                                seasonal_order=(1, 1, 1, 12),
                                enforce_stationarity=False,
                                enforce_invertibility=False)

nyeri_precipitation_results = nyeri_mod.fit()
...
# Fit the SARIMAX model using optimal parameters
print('Training the Nayuki weather Model')
nanyuki_mod = sm.tsa.statespace.SARIMAX(nanyuki_jioned_precipitation_df.precipitation_actual,
                                order=(1, 1, 1),
                                seasonal_order=(1, 1, 1, 12),
                                enforce_stationarity=False,
                                enforce_invertibility=False)

nanyuki_precipitation_results = nanyuki_mod.fit()


embu_model_name = 'embu_weather_model_prep'
nyeri_model_name = 'nyeri_weather_model_prep'
nanyuki_model_name = 'nayuki_weather_model_prep'
 
pickle.dump(embu_precipitation_results, open(embu_model_name, 'wb'))
pickle.dump(nyeri_precipitation_results, open(nanyuki_model_name, 'wb'))
pickle.dump(nanyuki_precipitation_results, open(nyeri_model_name, 'wb'))
